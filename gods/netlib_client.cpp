...
int32_t											nwol::initClientConnection					(::nwol::SClientConnection& conn)									{
	nwol_necall(::nwol::endpointCreate	(conn.l1, conn.l2, conn.l3, conn.l4, 0, &conn.pClient)						, "%s", "Error creating client connection.");
	nwol_necall(::nwol::endpointInit	(conn.pClient)																, "%s", "Error initializing client connection.");
	nwol_necall(::nwol::endpointBind	(conn.pClient)																, "Error binding client connection. Shutdown: 0x%x.", endpointShutdown(&conn.pClient));
	nwol_necall(::nwol::endpointCreate	(conn.r1, conn.r2, conn.r3, conn.r4, (uint16_t)conn.rPort, &conn.pServer)	, "%s", "Error creating connection to new port.");
	return 0;
}
...
int32_t											nwol::requestDisconnect						(::nwol::SClientConnection& conn)									{
	...
	nwol_necall(::nwol::endpointSend( conn.pClient, (const ubyte_t*)&commandDisconnect, (uint32_t)sizeof(::nwol::NETLIB_COMMAND), &bytesTransmitted, conn.pServer ), "%s", "Error sending disconnect message.");
...
int32_t											nwol::disconnectClient						(::nwol::SClientConnection& client)									{
	...
	::nwol::endpointShutdown(&client.pClient);
	::nwol::endpointShutdown(&client.pServer);
	return 0;
}
...
int												nwol::connect								(::nwol::SClientConnection& conn)									{
	...
	int32_t												bytesTransmitted							=-1;
	nwol_necall(::nwol::endpointSend(conn.pClient, (const ubyte_t*)&send_buffer, (uint32_t)sizeof(::nwol::NETLIB_COMMAND), &bytesTransmitted, conn.pServer), "Error transmitting data. Disconnect: 0x%X.", ::nwol::disconnectClient(conn));
	...
	::nwol::error_t										errMy										= ::nwol::endpointReceive(conn.pClient, (ubyte_t*)&connect_buffer, (uint32_t)sizeof(::nwol::NETLIB_COMMAND), &bytesTransmitted, 0);
	...
	::nwol::endpointShutdown(&conn.pServer);

	int32_t												port_buffer									= -1;
	errMy											= ::nwol::endpointReceive(conn.pClient, (ubyte_t*)&port_buffer, (uint32_t)sizeof(int32_t), &bytesTransmitted, 0);
	...
	nwol_necall(::nwol::endpointCreate(conn.r1, conn.r2, conn.r3, conn.r4, (uint16_t)port_buffer, &conn.pServer), "%s", "Error creating new server connection.");
