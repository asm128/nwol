...
namespace nwol
{
	...
	public:
		...
		template<typename _tRef>	inline	::nwol::error_t		getAttribute			(::nwol::id_t index, ::nwol::DATA_DIRECTION direction, ::nwol::gptr_nco<_tRef>& attributeStruct)										const				{ ree_if(0 == pGraph, "Uninitialized node."); return pGraph->GetAttribute	(Id, index, direction, attributeStruct);				}
		template<typename _tRef>	inline	::nwol::error_t		pushAttribute			(::nwol::DATA_DIRECTION direction, ::nwol::gptr_nco<_tRef>& attributeStruct, ::nwol::id_t* index, const ::nwol::glabel& description="")	const				{ ree_if(0 == pGraph, "Uninitialized node."); return pGraph->PushAttribute	(Id, direction, attributeStruct, index, description);	}
