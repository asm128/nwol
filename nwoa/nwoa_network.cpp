...
int											runCommunications						(::nwol::SApplicationNetworkClient& appNetwork)		{
	...
	while gbit_true(appNetwork.State, ::nwol::NETWORK_STATE_ENABLED) {
		break_error_if(result = ::nwol::ping(instanceClient.pClient, instanceClient.pServer) ? 0 : -1, "%s", "Ping timeout.");	// Ping before anything else to make sure everything is more or less in order.

		uint64_t										current_time;
		break_error_if(errored(result = ::nwol::serverTime(instanceClient, current_time)), "Failed to get server time.");		// get server time
...
::nwol::error_t								networkDisable							(::SApplication& instanceApp)						{
	...
	if(instanceAppNetwork.State, ::nwol::NETWORK_STATE_RUNNING)//(instanceAppNetwork.Connection.pClient && instanceAppNetwork.Connection.pServer)
