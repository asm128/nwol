...
static inline	::nwol::error_t								sdkLayersAvailable						()																{
...
static ::nwol::error_t										d3dCreateDevice							(::nwol::SD3D11& d3dToInitialize)							{
	...
	::nwol::com_ptr<::IDXGIDevice>									dxgiDevice								= {};
#if defined(_DEBUG)
	if not_errored(::sdkLayersAvailable())
...
::nwol::error_t												nwol::graphicsInitialize				(::nwol::SGraphics& instanceToInitialize)									{
	nwol_necall(::dxInitializeFactories		(instanceToInitialize.PlatformDetail), "Unknown reason");
	nwol_necall(::dxInitializeDevices		(instanceToInitialize.PlatformDetail), "Unknown reason");
	return 0;
}

::nwol::error_t												createWindowSizeDependentResources			(::nwol::SGraphics& instanceGraphics);
::nwol::error_t												updateForWindowSizeChange					(::nwol::SGraphics& instanceGraphics, const ::nwol::SCoord2<uint32_t>& currentSize)	{
...
::nwol::error_t												setDpi									(::nwol::SGraphics& instanceGraphics, const ::nwol::SCoord2<uint32_t>& currentSize, float dpi)	{
	::nwol::SD3D11												& d3d									= instanceGraphics.PlatformDetail.D3D;
	if (dpi != d3d.DPICurrent) {
		...
		updateForWindowSizeChange(instanceGraphics, currentSize);	// Often a DPI change implies a window size change. In some cases Windows will issue both a size changed event and a DPI changed event. In this case, the resulting bounds will not change, and the window resize code will only be executed once.
	}
	return 0;
}
...
::nwol::error_t												handleDeviceLost						(::nwol::SGraphics& instanceGraphics)														{
	...
	nwol_necall(::nwol::dxInitializeDevices(instanceGraphics.PlatformDetail), "Failed to create DirectX device resources.");
	//nwol_necall(::nwol::createDeviceResources(instanceGraphics), "Failed to create DirectX device resources.");
	return ::setDpi(instanceGraphics, {}, dpi);
}

::nwol::error_t												nwol::graphicsAttach					(::nwol::SGraphics& instanceGraphics, ::nwol::SDisplay& displayToAttachTo)					{
	::nwol::SD3D11												& d3d									= instanceGraphics.PlatformDetail.D3D;
	if (d3d.SwapChain != nullptr) {
		HRESULT															hr										= d3d.SwapChain->ResizeBuffers(2, 0, 0, DXGI_FORMAT_B8G8R8A8_UNORM, 0);	// If the swap chain already exists, resize it.
		if (hr == DXGI_ERROR_DEVICE_REMOVED) {
			//nwol_necall(::handleDeviceLost(instanceGraphics), "Unknown error.");	// If the device was removed for any reason, a new device and swapchain will need to be created.
			return 0;	// Everything is set up now. Do not continue execution of this method.
		}
		else { 
			nwol_hrcall(hr);
		}
		...
		nwol_hrcall(dxgiDevice->SetMaximumFrameLatency(1));
	}

	// Create a Direct3D render target view of the swap chain back buffer.
	::nwol::com_ptr<ID3D11Texture2D>												backBuffer;
	nwol_hrcall(d3d.SwapChain->GetBuffer(0, IID_PPV_ARGS(&backBuffer)));
	nwol_hrcall(d3d.Device->CreateRenderTargetView(backBuffer, nullptr, &d3d.RenderTargetView));

	// Cache the rendertarget dimensions in our helper class for convenient use.
	D3D11_TEXTURE2D_DESC															backBufferDesc								= {0};
	backBuffer->GetDesc(&backBufferDesc);
	d3d.RenderTargetSize.x														= backBufferDesc.Width;
	d3d.RenderTargetSize.y														= backBufferDesc.Height;
	...
	nwol_hrcall(d3d.Device->CreateDepthStencilView(depthStencil, &viewDesc, &d3d.DepthStencilView));
