...
namespace nwol
{
	...
	template <typename _tCell>					void				fillRectangleBorders	(_tCell* target, int32_t targetWidth, int32_t targetHeight, const _tCell& color, const ::nwol::SRectangle2D<int32_t>& rect)												{
	...
	template <typename _tCell>					void				fillCircle				(_tCell* target, int32_t targetWidth, int32_t targetHeight, const _tCell& color, const ::nwol::SCircle2D<int32_t>& circle)												{
		...
		for(int32_t y=ystart; y<ystop; ++y)
			for(int32_t x=xstart; x<xstop; ++x) {							
				const ::nwol::SCoord2<int32_t>										cellPos					= {x, y};
	...
	template <typename _tCell, typename _tCoord>	void			fillRectangle			(_tCell* target, int32_t targetWidth, int32_t targetHeight, int32_t targetPitch, const _tCell& color, const ::nwol::SRectangle2D<_tCoord>& rect)						{
		const int32_t														ystart					= ::nwol::max(0, nwol::min(targetHeight	, rect.Offset.y				));
		const int32_t														xstart					= ::nwol::max(0, nwol::min(targetWidth	, rect.Offset.x				));
		const int32_t														ystop					= ::nwol::max(0, nwol::min(targetHeight	, rect.Offset.y+rect.Size.y	));
		const int32_t														xstop					= ::nwol::max(0, nwol::min(targetWidth	, rect.Offset.x+rect.Size.x	));
	...
	static inline								void				fillRectangle			(_tCell* target, int32_t targetWidth, int32_t targetHeight, int32_t targetPitch, _tCell color, const ::nwol::SRectangle2D<_tCoord>& rect, ::nwol::ALIGN_SCREEN align)	{
		if(align == (::nwol::SCREEN_tValueOP_LEFT))
			...
			::nwol::realignRectangle(targetWidth, targetHeight, rect, rectAligned, align);
			::nwol::fillRectangle(target, targetWidth, targetHeight, targetPitch, color, rectAligned);
		}
	}
