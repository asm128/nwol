...
#pragma pack(push, 1)
	...
	public:
		...
		inline constexpr										grid_view				(_tValue* Data, uint32_t width, uint32_t height)		noexcept	: Data(Data), Size({(uint32_t)width, (uint32_t)height})		{}

		template <size_t _width, size_t _height>																								
		inline constexpr										grid_view				(const	_tValue (&Data)[_height][_width])				noexcept	: Data(Data), Size({(uint32_t)_width, (uint32_t)_height})	{}

		template <size_t _width, size_t _height>																								
		inline constexpr										grid_view				(		_tValue	(&Data)[_height][_width])				noexcept	: Data(Data), Size({(uint32_t)_width, (uint32_t)_height})	{}

		// Report error before dereferencing invalid index.
		inline				grid_view<_tValue>&					operator=				(const grid_view& other)								noexcept	= default;
		inline				array_view<const _tValue>			operator[]				(uint32_t row)									const				{ 
			throw_if(0 == Data		, "", "Uninitialized pointer."); 
			throw_if(row >= Size.y	, "", "%s: %u.", "Invalid grid row"	, row); 
			return array_view<const _tValue>(&Data[row * Size.x], Size.x);
		}

		// Report error before dereferencing invalid index.
		inline				array_view<_tValue>					operator[]				(uint32_t row)														{ 
			throw_if(0 == Data		, "", "Uninitialized pointer."); 
			throw_if(row >= Size.y	, "", "%s: %u.", "Invalid grid row"	, row); 
			return array_view<_tValue>(&Data[row * Size.x], Size.x);
		}
