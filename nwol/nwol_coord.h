...
#pragma pack(push, 1)
	...
	template<typename _tCoord> struct	SSpan			{ _tCoord Begin, End;								/**/ inline constexpr bool operator !=(const	SRange			<_tCoord>& other ) const noexcept { return !operator==(other); } /**/ inline constexpr bool operator==(const	SRange			<_tCoord>	& other) const noexcept { return Offset		== other.Offset		&& Count		== other.Count;							} };
...
#pragma pack(pop)
	...
	template <typename _tCoord> static constexpr			bool				in_range				(const	SCoord2<_tCoord>& value, const SCoord2<_tCoord>& rangeStart, const SCoord2<_tCoord>& rangeStop)		noexcept	{ return (value.x >= rangeStart.x) && (value.x < rangeStop.x) && (value.y >= rangeStart.y) && (value.y < rangeStop.y);	}
	template <typename _tCoord> static constexpr			bool				in_range				(const	SCoord3<_tCoord>& value, const SCoord3<_tCoord>& rangeStart, const SCoord3<_tCoord>& rangeStop)		noexcept	{ return (value.x >= rangeStart.x) && (value.x < rangeStop.x) && (value.y >= rangeStart.y) && (value.y < rangeStop.y) && (value.z >= rangeStart.z) && (value.z < rangeStop.z);	}
	...
	template <typename _tCoord>	static inline				void				coord2D_rotateInPlace	(		SCoord2<_tCoord>& coord, double theta)																noexcept	{ coord2D_rotateInPlace(coord, ::nwol::getSinCos(theta)); }
	template <typename _tCoord>	static inline				SCoord2<_tCoord>	coord2D_rotate			(const	SCoord2<_tCoord>& coord, double theta)																noexcept	{ return coord2D_rotate(coord, ::nwol::getSinCos(theta)); }
