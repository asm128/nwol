...
#pragma pack(push, 1)
	...
	void						fillCellsFromNoise		(SGrid<_tCell, _Width, _Height>& grid, const _tCell& value, int64_t seed, const _tCell& clearValue = ' ', int32_t diceFaces=10) {
	...
	template<typename _tCell, size_t _Width, size_t _Height>
	void						clearGrid				( SGrid<_tCell, _Width, _Height>& grid, const _tCell& clearValue = ' ') {
		_tCell						* cells					= &grid.Cells[0][0];
		for(uint32_t i=0, count=_Width*_Height; i<count; ++i )
			cells[i]					= clearValue;
	}
	...
	template<typename _tCell, size_t _Width, size_t _Height>
	void						drawGridBorders			( SGrid<_tCell, _Width, _Height>& display, const _tCell& value) {
	...
	int32_t						printfToGrid			( SGrid<_tCell, _Width, _Height>& display, int32_t offsetLine, int32_t offsetColumn, nwol::ALIGN_SCREEN align, const char* format, _Args&&... args ) {
		int32_t							actualX					= printfToRect(&display.Cells[0][0], _Width, _Height, offsetLine, offsetColumn, align, format, args...);
		return actualX;
	}

	template<typename _tCell, size_t _Width, size_t _Height, typename... _Args>
	int32_t						printfToGridColored( SGrid<_tCell, _Width, _Height>& display, SGrid<uint16_t, _Width, _Height>& textAttributes, uint16_t messageColor, int32_t offsetLine, int32_t offsetColumn, nwol::ALIGN_SCREEN align, const char* format, _Args&&... args ) {
