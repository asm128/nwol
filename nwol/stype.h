...
//		inline						stype&							operator	=				(const _tBase&& other)				noexcept	{ Value = other; return *this;	}
	...
	template <typename _tBase, ::nwol::GDATA_TYPE _F, ::nwol::GDATA_USAGE _USAGE = ::nwol::GUSAGE_NUMERIC> struct spod : public supertype<_tBase, _F, _USAGE>
	{
		...
		inline						uint32_t						read						(const void* fp)								{ retnul_msg_if(0 == fp, "Parameter is null!");	this->Value = *reinterpret_cast<const _tBase*>(fp);	return (uint32_t)sizeof(_tBase); }
		inline						uint32_t						write						(void* fp)					const				{ retnul_msg_if(0 == fp, "Parameter is null!");	*reinterpret_cast<_tBase*>(fp) = this->Value;		return (uint32_t)sizeof(_tBase); }
