...
namespace nwol
{
	...
	struct genum_definition {
		...
		inline constexpr									genum_definition	()																	: Name("Enum definition name not set.")						{};
		inline static		genum_definition<_T>&			get					()																	{
			static	genum_definition<_T>	valueRegistry;
			return valueRegistry;
		}
		inline static		_T								init				(const ::nwol::gsyslabel& enumName)									{
			...
			return (_T)~(_T)0;
							...
							::nwol::error_t					get_value			(const ::nwol::gsyslabel& name, _T& value)			const			{
			...
			return -1;
							...
							_T								get_value			(const ::nwol::gsyslabel& name)						const			{
			for(uint32_t i=0, count = Names.size(); i<count; ++i)
				if(name == Names[i])
					return Values[i];
			...
			return (_T)~(_T)0;
							...
							::nwol::error_t					get_value_by_index	(uint32_t index, _T& value)							const			{
			if( index < Values.size() ) {
				...
				return -1;
							...
							_T								get_value_by_index	(uint32_t index)									const			{
			retval_msg_if((_T)~(_T)0, index >= Values.size(), "Enumeration index out of range! Index: 0x%u.", index)
			return Values[index];
		}
			...
			return -1;
							...
							::nwol::error_t					get_value_index		(const _T& value, int32_t& index)					const			{
			for(uint32_t i=0, count = Names.size(); i<count; ++i)
				if(value == Values[i]) {
					index = (int32_t)i;
					return 0;
			...
			return -1;
							...
							int32_t							get_value_index		(const _T& value)									const			{
			for(uint32_t i=0, count = Names.size(); i<count; ++i)
				if(value == Values[i])
					return (int32_t)i;
			...
			return -1;
							...
							::nwol::error_t					get_value_label		(const _T& value, ::nwol::gsyslabel& name)			const			{
			...
			return -1;
							...
							::nwol::gsyslabel				get_value_label		(const _T& value)									const			{
			for(uint32_t i=0, count = Values.size(); i<count; ++i)
				if(value == Values[i])
					return Names[i];
			...
			return UNRESOLVED_ENUM_LABEL_STR;
							...
							::nwol::error_t					add_value			(const _T& value, const ::nwol::gsyslabel& name)					{
			for(uint32_t i=0, count = Values.size(); i<count; ++i)
				if(Values[i] == value) {
					if(Names[i] != name) {
	...
	struct genum_value
	{
		...
		inline constexpr									genum_value			(const genum_value&& other)										: Value(other.Value), Name(other.Name)												{}
		inline												genum_value			(const _T& value)												: Value((_T)value), Name(::nwol::genum_definition<_T>::get().get_value_name(value))	{}
		inline												genum_value			(const _T& value, const ::nwol::gsyslabel& name)				: Value((_T)value), Name(name)														{ ::nwol::genum_definition<_T>::get().add_value(value, name); }
		inline constexpr	operator						const	_T&			()													const		{ return Value; }
	};
...
#define GDEFINE_ENUM_TYPE(EnumName, IntType)																																							\
	...
	static inline			EnumName					operator &			(EnumName a, EnumName b)										{ return (EnumName)		(a & (IntType)b);				}	\
	static inline			EnumName					operator ~			(EnumName a)													{ return (EnumName)		(~(IntType)a);					}	\
	static inline			EnumName					operator ^			(EnumName a, EnumName b)										{ return (EnumName)		(a ^ (IntType)b);				}	\
	static inline			EnumName&					operator |=			(EnumName &a, EnumName b)										{ return (EnumName&)	( ((IntType&)a) |= (IntType)b); }	\
	static inline			EnumName&					operator &=			(EnumName &a, EnumName b)										{ return (EnumName&)	( ((IntType&)a) &= (IntType)b); }	\
	static inline			EnumName&					operator ^=			(EnumName &a, EnumName b)										{ return (EnumName&)	( ((IntType&)a) ^= (IntType)b); }	\
	static inline constexpr	EnumName					operator |			(EnumName a, EnumName b)										{ return (EnumName)		(a | (IntType)b);					}
...
#define GDEFINE_ENUM_VALUE_NOPREFIX(EnumName, EnumValue, ValueName)																										\
	static					const EnumName					ValueName																			= (EnumName)(EnumValue);	\
	static					const EnumName					__sei_##EnumName##_##ValueName														= (EnumName)::nwol::genum_value<EnumName>((EnumName)(EnumValue), #ValueName)
